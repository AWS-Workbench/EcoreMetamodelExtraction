 ...
 * class uses the {@link JavaMemberExtractor} and the {@link DataTypeExtractor}.
 * 
 * @author Timur Saglam
 */
public class JavaTypeExtractor {
	...
	/**
	 * Parses a list of potential external types. If the model is a external type,
	 * and an IType can be found, it will be added as an external ExtractedType to
	 * the model. An external type is either not part of the intermediate model or
	 * it is part of the intermediate model and is a inner type, which means he can
	 * be treated as a external type in the case that external types are not
	...
	/**
	 * Parses {@link IType}. Detects whether the type is a (abstract) class, an
	 * interface or an enumeration.
	 * 
	...
	/**
	 * Checks whether an {@link IType} inherits from the class
	...
	/**
	 * Extracts a external type if it can be found with
	 * {@link IJavaProject#findType(String)}.
	 */
	 ...
	 * @param extractedType is the {@link ExtractedType} which receives the
	...
	/**
	 * Adds a inner type which is a pseudo external type to the
	 * {@link IntermediateModel}. These are generally nested types, which are
	 * included in the intermediate model but not always generated. Therefore they
	 * can be external types.
	 */
