...
public class EClassifierGenerator {
	 ...
	 * Basic constructor.
	 * 
	 * @param model    is the {@link IntermediateModel} which is used to extract a
	 *                 metamodel.
	...
	/**
	 * Completes the generation of the {@link EClassifier} objects. Adds methods and
	 * attributes to {@link EClass} objects, adds type parameters and super
	 * interfaces and sorts the external types.
	 */
	...
	/**
	 * Generates an {@link EClassifier} from an ExtractedType, if the type was not
	 * already generated.
	 * 
	 * @param type is the {@link ExtractedType}.
	 * @return the {@link EClassifier}, which is either an {@link EClass}, an
	 *         {@link ExtractedInterface} or an {@link EEnum}.
	 */
	...
	/**
	 * Generates a root container {@link EClassifier}, which is a simple
	 * {@link EClass} with a root container containment reference.
	 * 
	...
	/**
	 * Adds the super class of an extracted class to a specific {@link EClass}. If
	 * the extracted class has no super class, no EClass is added.
	 */
	...
	/**
	 * Adds all super interfaces of an extracted type to a specific {@link EClass}.
	 * If the extracted type has no super interfaces, no EClass is added.
	 */
	...
	/**
	 * Generates (if allowed) an {@link EClassifier} from an name which refers to a
	 * {@link ExtractedType} in the model and adds it as super type to an
	 * {@link EClass}.
	 */
	private void addSuperType(ExtractedDataType superType, EClass eClass) {
		...
		if (eClassifierMap.containsKey(superTypeName)) { // if is already created:
			...
			if (selector.allowsGenerating(extractedType)) { // is super type can be generated
				generateSuperRelation(eClass, (EClass) generateEClassifier(extractedType), superType); // Modified by
																										// Raman
			}
	...
	/**
	 * Generates an EClass from an extractedType (should be ExtractedClass or
	 * ExtractedInterface).
	 */
	...
	/**
	 * Generates a super type relation from an {@link EClass} to another
	 * {@link EClass}, with the help of an {@link ExtractedDataType} of the super
	 * type.
	 */
